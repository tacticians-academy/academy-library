...
import { SET_DATA, type SetNumber } from '../../dist/index.js'
...
import { githubTokenPath, loadHardcodedTXT } from '../helpers/files.js'
...
function wikiUrlFor(setNumber: SetNumber, { name }: { name: string }) {
	return `https://leagueoflegends.fandom.com/wiki/${name.replace(' ', '_')}/TFT#Set_${setNumber}`
}

export async function createGithubIssuesFor(setNumber: SetNumber) {
	const milestone = SET_DATA[setNumber].milestoneID
	if (milestone == null) {
		console.log('Unknown milestone', setNumber)
		return
	}
	...
	const { completedItems } = await importItems(setNumber)
	...
	for (const champion of champions) {
		const canContinue = await createIssue({
			...
			labels: [`Champion Ability`, `Set ${setNumber}`],
			milestone,
		})
		if (!canContinue) {
			break
